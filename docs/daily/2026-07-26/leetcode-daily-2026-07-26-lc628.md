---
title: "[力扣每日一题] 2026-07-26｜LC 628 三个数的最大乘积"
---

# [力扣每日一题] 2026-07-26｜LC 628 三个数的最大乘积

<p class="daily-archive-kicker">2026-07-26 · 第 14/14 题 · 力扣每日一题</p>

<p class="daily-archive-utility"><a href="index.md">返回 2026-07-26 题目列表</a> · <a href="../../basics/signed-product-extrema.md">进入知识专题</a></p>
...
- 官方中文标题：三个数的最大乘积
- 官方难度：简单
- 官方链接：<https://leetcode.cn/problems/maximum-product-of-three-numbers/>

### 原始题意

给定整数数组 `nums`，从三个不同位置各选择一个数，返回这三个数的最大乘积。数组允许负数、零、正数和重复值。
...
- $3\le |\texttt{nums}|\le 10^4$。
- $-1000\le \texttt{nums}[i]\le 1000$。
- 任意三数乘积位于 $[-10^9,10^9]$，`int` 足够。

## 最优结论

一次扫描同时维护三个最大值 $h_1\ge h_2\ge h_3$ 与两个最小值 $l_1\le l_2$，最终答案为

$$
\max(h_1h_2h_3,\;l_1l_2h_1).
$$

时间复杂度为 $O(n)$，额外空间复杂度为 $O(1)$，并达到必须读取全部输入的 $\Omega(n)$ 下界。优先记忆五变量扫描：它不修改输入、常数小，也准确揭示负数使答案同时依赖数轴两端。
...
每个新数分别插入最大值链和最小值链。结束时，两个可能的最优三元组所需信息都仍在状态中；完整实现如下。
...
- [LeetCode 628：三个数的最大乘积](https://www.wineandchord.com/algo/problems/#problem-lc-628)
- [std::multiset — cppreference](https://en.cppreference.com/w/cpp/container/multiset)

### 延伸阅读

- [官方题目](https://leetcode.cn/problems/maximum-product-of-three-numbers/)
- [对应知识专题](../../basics/signed-product-extrema.md)

<nav class="daily-archive-pager" aria-label="当日题目导航">
<a class="daily-archive-pager__previous" href="codeforces-2247-a.md">← [codeforces] CF Round 1111 Div.2 A Zero Sum</a>
