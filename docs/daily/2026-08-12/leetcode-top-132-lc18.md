...
<!-- DAILY_CANONICAL_BODY_START sha256=0eb34f5b3bc707763dec36ab161c2f05dfff986cafbfdcc04f96130a73d776f4 -->
## 官方原始信息
...
任意合法值四元组按非降序排列后，都对应某组 $i<j<\mathrm{left}<\mathrm{right}$。外层只跳过相同的首值或次值，保留其第一次出现；双指针又不会跨过仍可能达到目标的组合，所以它必被找到。每层均跳过相同值，因此答案不重复。算法既无遗漏又无重复，结论成立。
