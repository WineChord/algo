...
若每个输出字符只由同位置输入字符决定，且输出长度与顺序都不变，就不需要前缀状态、窗口或
字符统计。一次原地扫描能直接完成映射；正确性也可逐位置证明。

这里最常见的误建模是“筛选所有满足条件的字符”，它会缩短字符串。题目要求不满足条件的
位置输出占位符时，位置本身也是答案的一部分。只有出现多次区间查询、在线修改或压缩输入，
才需要把模型升级为前缀计数、树状数组或游程级处理。

--8<-- "includes/problems/atcoder-abc472-a.md"

## 固定宽度分块：整体反转同时反转块序与块内位序

把每个字符编码为固定 $b$ 位后再拼接，整体回文的对称位置会落到倒序块中的反向位。
因此无需构造长比特串：第 $i$ 个块必须等于倒数第 $i$ 个块的 $b$ 位反转。固定块宽保证
边界不会错位；若编码是变长的，就必须先保留边界或直接比较完整编码流。
...
若区间只要求某个目标字符恰好出现 $k$ 次，任意最短合法区间都不会保留首尾的非目标字符。
记录目标字符位置 $p_0,p_1,\ldots$ 后，所有最短候选只可能是
$[p_i,p_{i+k-1}]$。这把左右端点的平方枚举压成目标位置上的线性窗口；若还要比较候选内容，
字符串复制与字典序比较可能重新带来平方时间，规模放大时可用后缀数组或 LCP 结构。

--8<-- "includes/problems/lc-2904.md"

## 多重集合的字典序后继：最靠右可增大的枢轴

用固定字符频次重排字符串、求严格大于目标的最小结果时，任意答案都有唯一的首次不同位置：
此前前缀与目标相同，该位严格更大。首次差异越靠右，整个字符串越小；固定差异位置后，应选
最小可用的更大字符，并把剩余多重集合升序排成最小后缀。

实现时先消耗频次尽量匹配目标前缀。当前位没有更大字符，就向左回退一位并恢复刚才消耗的
字符，直到找到可增大的最靠右枢轴。完整匹配目标仍不满足“严格大于”，同样必须回退。这个
过程只需 $26$ 个频次；若字符表很大，可用有序映射查询后继。若每个位置还有独立字符限制，
升序后缀不再自动可行，需要额外的匹配或流判定。
