...
### 数字和与数字积的整除聚合 { #digit-sum-product-divisibility }

对固定进制的整数，反复取模得到末位、整除进制删去末位，可以在一次扫描中同时维护多种可交换
的数位聚合。数字和的单位元是 0，数字积的单位元是 1；输入中的数字 0 会把积吸收到 0，不能
被当作“没有数位”跳过。

整除判断还要保留原数，用副本拆位。正整数的数字和至少为 1，因此即使数字积为 0，
`sum + product` 仍不会成为零除数。若输入扩展到机器整数之外，数位和仍小，但数字积与被除数
都可能需要大整数或十进制长除法；此时模型不变，只有表示层升级。

--8<-- "includes/problems/lc-3622.md"

### 无限数字串：先跳等位数段，再恢复块内位置

无限拼接整数时，位置 $k$ 很大并不意味着必须生成前 $k$ 个字符。先按十进制位数分层：
$d$ 位正整数共有 $9\times10^{d-1}$ 个，对应字符数为
$9\times10^{d-1}d$。删去完整位数段后，剩余位置除以 $d$ 就能恢复目标整数序号，取模则得到
整数内的数位下标。

若每个固定大小的整数块会升序、降序或按周期置换，位数段的总字符数仍不变；只需在最后一步
把全局整数序号映射到块号与块内偏移，再按该块的排列规则反解真实整数。这个分层把“无限串”
拆成稳定的计数层与局部置换层，也清楚隔离了最容易混淆的 1-based 位置和 0-based 下标。
