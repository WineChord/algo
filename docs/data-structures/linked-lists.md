...
若只反转第 `left` 到第 `right` 个节点，先让哨兵后的指针停在区间前驱。固定区间原首节点作为尾，每轮把它的后继摘下并插到区间最前；区间外边界始终保持连通。
...
删除所有目标值时，`prev` 始终指向已经过滤完成前缀的最后一个节点。若 `prev->next` 应删除，只改写这条边而不移动 `prev`；否则才让 `prev` 前进。连续目标值和删除原头因此都落入同一分支。
...
若目标不是比较两半，而是把首尾交替接回，则在找到中点后先断开前后两段，反转后半，再做拉链合并。断链不可省略：否则原中点附近仍保留旧边，合并过程可能形成环。奇数长度时前半多一个节点，合并循环自然把它留在末尾。

--8<-- "includes/problems/lc-143.md"

## 首尾成环：把循环位移变成一次切断

链表右旋若逐次把尾节点移到头部，单次需要寻找前驱，总时间会达到 $O(nk)$。先求长度并令 $k\bmod n$，再把尾节点连回头节点形成环；新尾位置一旦确定，只需断开一条边即可得到目标链表。

成环只是临时建模，返回前必须保证恰有一条边被断开。空链、单节点和整圈位移应在成环前直接返回，避免制造无意义的自环或遗漏断链。
