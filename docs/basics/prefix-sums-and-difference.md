...
若只能在相邻元素之间切一刀，切口左侧和为 $P$、总和为 $S$，两侧差值可以直接化为

$$
|P-(S-P)|=|2P-S|.
$$

因此无需为每个切口重新求左右区间和：先求一次 $S$，再让 $P$ 随切口移动增量更新即可。
合法切口只有 $1\ldots n-1$；把空前缀或整段也纳入候选会改变题意。若元素允许负数，
$P$ 不再单调，但线性枚举全部切口仍然正确；只有进一步要求切成多个正权连续段时，才会出现
二分答案或更高维动态规划。
...
第一遍从左到右把严格左侧乘积写入输出数组；第二遍从右到左维护一个滚动后缀积并乘入。输出数组本身承载前缀状态，因此除结果外只需常数空间。相同结构也可迁移到任意满足结合律的聚合：若还需要保持顺序，左右两侧的合并顺序必须与运算定义一致。
