...
若每行内部递增，且下一行首元素严格大于上一行末元素，矩阵按行展开后就是一条全局递增序列。二分状态只需保存虚拟下标 $k$，访问时映射到

$$
\left(\left\lfloor\frac{k}{n}\right\rfloor,\ k\bmod n\right).
$$

这不会真的复制矩阵；它只是把有序性和存储坐标分开。若只保证每行、每列分别有序，跨行全局次序不成立，应改用右上角收缩或其他二维单调算法。
...
二分不一定要求数组值整体单调。若比较相邻位置能证明某一侧至少存在一个目标，就可以维护“当前闭区间内至少有一个解”的存在性不变量。寻找峰值时，中点到右邻若上升，右半必在边界或首次转折处出现峰值；否则左半（含中点）必有峰值。每次都能安全舍弃一半区间。
...
要最大化区间中胜利标记的比例，候选值 $p$ 是否可达具有单调性。把一次胜利记为 $1-p$、一次失败记为 $-p$ 后，某区间胜率至少为 $p$ 当且仅当其变换和非负。

若区间还要求至少包含 $K$ 次胜利，合法左端点集合会随右端点单调扩张。对每个右端点维护“已合法前缀中的最小前缀和”，即可在线性时间完成一次判定。答案二分因此不是套模板：真正的工作在于把分式约束改写成可线性检查的前缀不等式。
